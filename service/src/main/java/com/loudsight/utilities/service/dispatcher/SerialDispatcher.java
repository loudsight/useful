...
public class SerialDispatcher implements Dispatcher {
...
////    }
    ...
    @Override
    public <Q, A> Subscription<Q, A> subscribe(Address to, MessageHandler<A> handler) {
        return newSubscription(to, new MessageHandler<A>() {
            @Override
            public A onMessage(Address to, Address replyTo, Subject recipient, Subject sender, BridgeMessageType publicationType, Object payload) {
    ...
    @Override
    public <T> Subscription<Publication, T> bridge(Address to, MessageHandler<T> handler) {
        return this.<Publication, T>newSubscription(to, new MessageHandler<T>() {
            @Override
            public T onMessage(Address to,
                                  Address replyTo,
                                  Subject recipient,
                                  Subject sender,
                                  BridgeMessageType publicationType,
    ...
    private <Q, A> Subscription<Q, A> newSubscription(Address to, MessageHandler<A> handler,
                                                      Boolean isBridged
    ) {
...
//            }
            ...
            @Override
            public A onEvent(Address to, Address replyTo, Subject recipient, Subject sender, Object payload, BridgeMessageType publicationType) {
...
//                println("Remove subscription {id: $id to: $to isBridged: $isBridged} ")
    ...
    @Override
    public <T> void publishAsync(Address to,
                             Subject recipient,
                             Subject publisher,
                             Object payload,
        ...
        var replyTo =
                new Address("com.loudsight.utilities.service.dispatcher.SerialDispatcher.publishAsync",
                        "" + replyId++);

        var subscription = this.subscribe(replyTo, new MessageHandler<T>() {
            @Override
            public T onMessage(Address to, Address replyTo, Subject recipient, Subject sender, BridgeMessageType publicationType, Object payload) {
